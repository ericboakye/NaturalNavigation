 ...
 */
@PluginDependant
public class HomeListCommand extends SubCompartment<NavigationCommand> {

    private NaturalNavigation plugin;

    public HomeListCommand(NaturalNavigation plugin) {
        super("/nav homelist <player>", "homelist");
        this.plugin = plugin;
        this.setPermission("nav.command.homelist");
    }
    ...
    public void execute(CommandSender sender, String[] args) throws CommandException {
        ...
        try {
            ...
            int homes = plugin.getHomesStorage().getHomes(data).size();
            if (homes == 0) {
                Message.create("noHomesList").param(args[0]).sendTo(sender);
                return;
            }

            sender.sendMessage(ChatColor.GOLD + "Available homes: ");
            for (int i = 0; i < homes; i++)
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&', i + "&6:&f " + plugin.getHomesStorage().getHomes(data).get(i).getName()));
