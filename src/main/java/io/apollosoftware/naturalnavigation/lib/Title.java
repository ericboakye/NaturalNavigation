package io.apollosoftware.naturalnavigation.lib;
...
public class Title {
    ...
    public void sendActionBar(Player receivingPacket) {
        try {

            Object serialized = nmsChatSerializer.getMethod("a", String.class).invoke(null, "{\"text\": \"" + title + "\"}");
            Object packet;


            if (!getVersion().contains("1_7")) {
                packet = packetType.getConstructor(chatBaseComponent, byte.class).newInstance(serialized, (byte) 2);
            } else {
                packet = packetType.getConstructor(chatBaseComponent, int.class).newInstance(serialized, 2);
            }

            Object handle = getHandle(receivingPacket);
